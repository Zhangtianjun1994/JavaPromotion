...
public class DL_quarterTest {
public static void main(String[] args) {
		
		FJ_OF_Month fj = new FJ_OF_Month();	
		System.out.println("开始测试GM模型的应用");
		System.out.println("*****************对高新园区2016年的房价进行相关的预测***************");
		System.out.println("输入第开始的季度");
		Scanner scanner = new Scanner(System.in);
		String start = scanner.nextLine();
		System.out.println("请输入结束季度");
		String end = scanner.next();
		System.out.println(start + end);
		//返回2008-2016年的平均房价序列
		double list_read[] = fj.return_DL_quarter_Avgprice(start,end);
		int num_GM = 0;
		//int index = -1;
...
//	    //根据Gm模型预测，得到预测序列
		...
		System.out.println("预测的下一季度房价："+nextYear/num_GM);
